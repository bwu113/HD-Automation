...
def insertExcel(PackingList):
    ...
    # rowRange = wks.range((1,1),(1,2)) # [Row, Column]

    # print(rowRange[0].column)
    ...
    #Order Information
            ...
            if("sku2" in item):
                ...
                rangeList = wks.range((currRow+11,currCol+1),(currRow+15,currCol+1))
                for row in rangeList:
                    if not row.value:
                        wks.range(row.row,currCol-2).value = [item["qty2"],"pcs",item["sku2"].rstrip("-"),item["desc2"], untPrice, totalPrice]
                        break
                if("bs" in item and "mir" in item):
                    wks.range(currRow+11,currCol-1).value = [item["qty"],"pcs","BS","Matching Backsplash","Inclusive",""]
                    wks.range(currRow+12,currCol-1).value = [item["qty"],"pcs","MIR","Matching Mirror","Inclusive",""]
